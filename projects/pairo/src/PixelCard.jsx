...
export default function PixelCard({
  ...
  useEffect(() => {
    ...
    // Auto-play: trigger appear on mount
    if (autoPlay) {
      // Small delay so canvas is ready
      const timer = setTimeout(() => handleAnimation('appear'), 50);
      return () => {
        clearTimeout(timer);
        cancelAnimationFrame(animationRef.current);
      };
  ...
  }, [gap, speed, colors, autoPlay]);

  return (
    <div
      ref={containerRef}
      className={`pixel-card ${className}`}
      onMouseEnter={!autoPlay ? onMouseEnter : undefined}
