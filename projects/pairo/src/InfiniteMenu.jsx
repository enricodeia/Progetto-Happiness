...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0, autoRotate = false, autoRotateSpeed = 0.3) {
    this.canvas = canvas
    this.items = items || []
  ...
  #initTexture() {
    ...
    // For items that have color strings instead of image URLs, draw colored circles with initials
    this.items.forEach((item, i) => {
      ...
      if (item.image && item.image.startsWith('#')) {
        ...
        ctx.fillStyle = item.image
        ctx.fillRect(x, y, cellSize, cellSize)
        // Draw initials
        ctx.fillStyle = 'rgba(255,255,255,0.15)'
        ctx.font = `bold ${cellSize * 0.35}px Space Grotesk, sans-serif`
        ctx.textAlign = 'center'
        ctx.textBaseline = 'middle'
        ctx.fillText(item.initials || '', x + cellSize / 2, y + cellSize / 2)
      }
    })

    // Upload initial canvas (colored placeholders)
    gl.bindTexture(gl.TEXTURE_2D, this.tex)
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, canvas)
    gl.generateMipmap(gl.TEXTURE_2D)

    // Load any real images asynchronously
    const imageItems = this.items.filter(item => item.image && !item.image.startsWith('#'))
  ...
  #animate(deltaTime) {
    ...
    if (this.autoRotate && !this.control.isPointerDown) {
      const angle = this.autoRotateSpeed * deltaTime * 0.001
      const autoQuat = quat.create()
      quat.setAxisAngle(autoQuat, [0.3, 1, 0.1], angle)
      this.control.orientation = quat.multiply(quat.create(), autoQuat, this.control.orientation)
...
export default function InfiniteMenu({ items = [], scale = 1.0, onSelect, autoRotate = false, autoRotateSpeed = 0.3 }) {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(canvas, items.length ? items : [], handleActiveItem, setIsMoving, sk => sk.run(), scale, autoRotate, autoRotateSpeed)
    }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <div className="globe-active-info">
            <span className={`globe-active-name ${isMoving ? 'inactive' : 'active'}`}>{activeItem.title}</span>
            <span className={`globe-active-role ${isMoving ? 'inactive' : 'active'}`}>{activeItem.description}</span>
          </div>

          <div
            onClick={() => onSelect && onSelect(activeItem)}
            className={`globe-select-button ${isMoving ? 'inactive' : 'active'}`}
          >
