...
uniform float uCurlStrength1;
uniform float uCurlStrength2;
uniform float uSmokeDecay;
uniform vec3 uSmokeColor1;
uniform vec3 uSmokeColor2;
uniform float uSmokeGlowPower1;
uniform float uSmokeGlowPower2;
...
void main() {
  ...
  vec2 uv2 = uv + curlNoise(vec3(uv * uCurlScale2 + uTime * uCurlSpeed, uTime * uCurlSpeed)).xy * uDt * uCurlStrength2;
  uv += curlNoise(vec3(uv * uCurlScale1 + uTime * uCurlSpeed, uTime * uCurlSpeed)).xy * uDt * uCurlStrength1;
  ...
  t2 = pow(t2, uSmokeGlowPower1);
  t3 = pow(t3, uSmokeGlowPower2);
  float scale = speed * 5.;
  t *= scale;
  t2 *= scale;
  t3 *= scale;

  color = mix(color, uSmokeColor1, t);
  color = mix(color, uSmokeColor2, t3);
