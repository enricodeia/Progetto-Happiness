...
export default function AppD() {
  ...
  const versionB = useControls({
    'D · Logo Intro': folder({
      ...
      logoIntroDurationS: { value: 1.75, min: 0.1, max: 4, step: 0.05, label: 'duration s' },
      logoIntroDelayS:    { value: 0.25, min: 0, max: 3, step: 0.05, label: 'start delay s' },
      logoIntroStaggerS:  { value: 0.05, min: 0, max: 0.6, step: 0.01, label: 'stagger per piece s' },
      logoIntroEase: {
        value: 'expo.out',
    ...
    'D · Smoky Ocean Material': folder({
      ...
      smokyNormalScale:          { value: 1.45,  min: 0, max: 4, step: 0.01, label: 'normal scale' },
      smokyRoughness:            { value: 0.85,  min: 0, max: 1, step: 0.01, label: 'roughness' },
      smokyMetalness:            { value: 0.00,  min: 0, max: 1, step: 0.01, label: 'metalness' },
      smokyBumpScale:            { value: 0.032, min: 0, max: 0.5, step: 0.001, label: 'bump scale' },
      smokyTransmission:         { value: 0.490, min: 0, max: 1, step: 0.01, label: 'transmission' },
      smokyThickness:            { value: 0.000, min: 0, max: 5, step: 0.01, label: 'thickness' },
      smokyIor:                  { value: 1.39,  min: 1, max: 2.333, step: 0.005, label: 'ior' },
      smokyClearcoat:            { value: 1.00,  min: 0, max: 1, step: 0.01, label: 'clearcoat' },
      smokyClearcoatRoughness:   { value: 1.00,  min: 0, max: 1, step: 0.01, label: 'clearcoat roughness' },
      smokySheen:                { value: 1.00,  min: 0, max: 1, step: 0.01, label: 'sheen' },
      smokySheenRoughness:       { value: 0.30,  min: 0, max: 1, step: 0.01, label: 'sheen roughness' },
      smokySheenColor:           { value: '#ffe0c4', label: 'sheen color' },
      smokyAnisotropy:           { value: 0.00,  min: 0, max: 1, step: 0.01, label: 'anisotropy' },
      smokyAnisotropyRotation:   { value: 0.00,  min: 0, max: Math.PI, step: 0.01, label: 'anisotropy rotation' },
      smokyIridescence:          { value: 0.00,  min: 0, max: 1, step: 0.01, label: 'iridescence' },
      smokyIridescenceIOR:       { value: 1.17,  min: 1, max: 2.333, step: 0.01, label: 'iridescence IOR' },
      smokyIridescenceMin:       { value: 233,   min: 0, max: 1000, step: 1, label: 'irid. thickness min nm' },
      smokyIridescenceMax:       { value: 431,   min: 0, max: 2000, step: 1, label: 'irid. thickness max nm' },
      smokyAttenuationDistance:  { value: 2.25,  min: 0.01, max: 20, step: 0.01, label: 'attenuation dist' },
      smokyAttenuationColor:     { value: '#ffffff', label: 'attenuation color' },
      smokySpecularIntensity:    { value: 0.45,  min: 0, max: 1, step: 0.01, label: 'specular intensity' },
      smokySpecularColor:        { value: '#ffffff', label: 'specular color' },
      smokyEmissive:             { value: '#000000', label: 'emissive' },
      smokyEmissiveIntensity:    { value: 0.00,  min: 0, max: 5, step: 0.01, label: 'emissive intensity' },
    }, { collapsed: false }),
  ...
  // Version D is an experimental variant — Leva is toggleable with "c" so we
  // can tune the responsive camera. 1/2/3/4 nav shortcuts as usual.
  ...
  // no matter the aspect ratio. With a near-orthographic vertical FOV of 5°,
  // the visible WIDTH at a given distance D is:
  //     visibleWidth = 2 * D * aspect * tan(vFov / 2)
  // So for a desired logo width W we solve D:
  //     D = W / (2 * aspect * tan(vFov / 2))
  // The 4-arm logo's X span is ≈ 2.88 (default targetSize 1.45 × 2).
  ...
  useEffect(() => {
    ...
    const computeDistance = () => {
      ...
      const targetWidth = responsive.logoWorldWidth / Math.max(0.001, responsive.logoFillVw)
      const d = targetWidth / (2 * aspect * Math.tan(vFovRad / 2))
      setCam({ distance: +d.toFixed(3) })
    }
    computeDistance()
    window.addEventListener('resize', computeDistance)
    return () => window.removeEventListener('resize', computeDistance)
  }, [responsive.responsiveOn, responsive.logoWorldWidth, responsive.logoFillVw, cam.fov, setCam])
        ...
        <Suspense fallback={null}>
          <AppBCanvas
            config={{
              ...
              // Smoky Ocean PBR material — built inside AppBCanvas where
              // three.js is already in scope. Skip pass-through if disabled.
              smokyOcean: versionB.smokyOn ? {
                on: true,
                repeat:                versionB.smokyRepeat,
                displacementScale:     versionB.smokyDisplacementScale,
                displacementBias:      versionB.smokyDisplacementBias,
                normalScale:           versionB.smokyNormalScale,
                roughness:             versionB.smokyRoughness,
                metalness:             versionB.smokyMetalness,
                bumpScale:             versionB.smokyBumpScale,
                transmission:          versionB.smokyTransmission,
                thickness:             versionB.smokyThickness,
                ior:                   versionB.smokyIor,
                clearcoat:             versionB.smokyClearcoat,
                clearcoatRoughness:    versionB.smokyClearcoatRoughness,
                sheen:                 versionB.smokySheen,
                sheenRoughness:        versionB.smokySheenRoughness,
                sheenColor:            versionB.smokySheenColor,
                anisotropy:            versionB.smokyAnisotropy,
                anisotropyRotation:    versionB.smokyAnisotropyRotation,
                iridescence:           versionB.smokyIridescence,
                iridescenceIOR:        versionB.smokyIridescenceIOR,
                iridescenceMin:        versionB.smokyIridescenceMin,
                iridescenceMax:        versionB.smokyIridescenceMax,
                attenuationDistance:   versionB.smokyAttenuationDistance,
                attenuationColor:      versionB.smokyAttenuationColor,
                specularIntensity:     versionB.smokySpecularIntensity,
                specularColor:         versionB.smokySpecularColor,
                emissive:              versionB.smokyEmissive,
                emissiveIntensity:     versionB.smokyEmissiveIntensity,
              } : null,
            }}
          />
        </Suspense>
